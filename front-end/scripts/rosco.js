...
function empezarJuego() {
  var dificultad = document.getElementById("dificultadElegida").value;
...
function siguienteConDatosDesde(desde) {
  for (let paso = 1; paso <= letras.length; paso++) {
    ...
    const letra = letras[i];
    if (estados[letra] === "pendiente" && preguntasPorLetra[letra]) return i;
  }
  return null;
}
...
function comprobarRespuesta() {
  ...
  const datos = preguntasPorLetra[letra];
  const respuestaUsuario = document.getElementById("rosco-respuesta").value.trim().toLowerCase();
